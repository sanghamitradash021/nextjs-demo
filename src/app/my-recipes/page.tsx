...
async function getServerSideRecipes() {
  try {
    ...
    if (!token || !userCookie) {
      return { error: 'User not authenticated.', recipes: [] };
    }

    const user = JSON.parse(userCookie);

    const recipes = await fetchUserRecipes(user.id, token);
...
export default async function MyRecipesPage() {
  ...
  const { recipes, error } = await getServerSideRecipes();
  ...
  return (
    <ThemeProvider>
      <RecipeList initialRecipes={recipes} initialError={error} />
    </ThemeProvider>
  );
}
